...
c = gmsh.model.geo.add_point(4.448807667, 0.0, 0.0, ff_size)
d = gmsh.model.geo.add_point(0.554227, 1.760289, 0.0, ff_size)
e = gmsh.model.geo.add_point(3.1995, 0.0, 0.0, ff_size)
...
ov = gmsh.model.geo.revolve(
    [(2, ps)], 0, 0, 0, 1, 0, 0, 2 * math.pi / 3, [21], recombine=True
)
ov = gmsh.model.geo.revolve(
    [(2, 40)], 0, 0, 0, 1, 0, 0, 2 * math.pi / 3, [21], recombine=True
)
ov = gmsh.model.geo.revolve(
    [(2, 72)], 0, 0, 0, 1, 0, 0, 2 * math.pi / 3, [21], recombine=True
)

gmsh.model.geo.add_physical_group(
    2, [20, 24, 28, 31, 52, 56, 60, 63, 84, 88, 92, 95], name="capsule"
)
gmsh.model.geo.add_physical_group(2, [35, 67, 99], name="outflow")
gmsh.model.geo.add_physical_group(2, [38, 70, 102], name="inflow")
