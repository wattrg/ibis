...
public:
    ...
    void symbolic_phase(Ibis::CrsGraph<Offset, Ordinal>& A, int fill_level) {
        ...
        L.graph.row_map = Ibis::Array1D<Offset, Layout, MemSpace>("L_row_map", num_rows + 1);
        L.graph.entries = Ibis::Array1D<Ordinal, Layout, MemSpace>("L_entries", spiluk_handle->get_nnzL());
        L.values = Ibis::Array1D<Scalar, Layout, MemSpace>("L_values",  spiluk_handle->get_nnzL());
        U.graph.row_map = Ibis::Array1D<Offset, Layout, MemSpace>("U_row_map", num_rows + 1);
        U.graph.entries = Ibis::Array1D<Ordinal, Layout, MemSpace>("U_entries", spiluk_handle->get_nnzU());
        U.values = Ibis::Array1D<Scalar, Layout, MemSpace>("U_values",  spiluk_handle->get_nnzU());

        KokkosSparse::spiluk_symbolic(&handle_, fill_level_,
                                      A.row_map, A.entries,
                                      L.graph.row_map, L.graph.entries,
                                      U.graph.row_map, U.graph.entries);
    }
    ...
    void numeric_phase(Ibis::CrsMatrix<Offset, Ordinal, Scalar>& A) {
        KokkosSparse::spiluk_numeric(&handle_, fill_level_,
                                     ...
                                     L.graph.row_map, L.graph.entries, L.values,
                                     U.graph.row_map, U.graph.entries, U.values );
    }
