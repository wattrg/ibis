...
void ausmdv(FlowStates<T>& left, FlowStates<T>& right, ConservedQuantities<T>& flux, bool three_d){
    Kokkos::parallel_for("ausmdv", flux.size(), KOKKOS_LAMBDA(const int i){
        T rL = left.gas.rho(i);
...
template void ausmdv<double>(FlowStates<double>&, FlowStates<double>&, ConservedQuantities<double>&, bool);
