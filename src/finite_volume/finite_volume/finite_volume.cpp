    ...
    for (size_t boundary_i = 0; boundary_i < grid.other_blocks().size(); boundary_i++) {
        ...
        Ibis::parallel_for(
            ...
            KOKKOS_LAMBDA(const size_t cell_i) {
                ...
                size_t var_i = 0;
                if (transfer_p) {
                    add_gradient_to_buffer_(buffer, cell_grad.p, start_index,
                                            cell_to_pack, var_i, dim);
                    var_i++;
                }
                if (transfer_rho) {
                    add_gradient_to_buffer_(buffer, cell_grad.rho, start_index,
                                            cell_to_pack, var_i, dim);
                    var_i++;
                }
                if (transfer_u) {
                    add_gradient_to_buffer_(buffer, cell_grad.u, start_index,
                                            cell_to_pack, var_i, dim);
                    var_i++;
                }
                if (transfer_temp) {
                    add_gradient_to_buffer_(buffer, cell_grad.temp, start_index,
                                            cell_to_pack, var_i, dim);
                    var_i++;
                }
                if (transfer_vx) {
                    add_gradient_to_buffer_(buffer, cell_grad.vx, start_index,
                                            cell_to_pack, var_i, dim);
                    var_i++;
                }
                if (transfer_vy) {
                    add_gradient_to_buffer_(buffer, cell_grad.vy, start_index,
                                            cell_to_pack, var_i, dim);
                    var_i++;
                }
                if (transfer_vx) {
                    add_gradient_to_buffer_(buffer, cell_grad.vz, start_index,
                                            cell_to_pack, var_i, dim);
                    var_i++;
                }
            });
    ...
    for (size_t boundary_i = 0; boundary_i < grid.other_blocks().size(); boundary_i++) {
        ...
        Ibis::parallel_for(
            ...
            KOKKOS_LAMBDA(const size_t cell_i) {
                ...
                size_t var_i = 0;
                if (transfer_p) {
                    unpack_gradient_from_buffer_(buffer, cell_grad.p, start_index,
                                                 cell_to_unpack_to, var_i, dim);
                    var_i++;
                }
                if (transfer_u) {
                    unpack_gradient_from_buffer_(buffer, cell_grad.u, start_index,
                                                 cell_to_unpack_to, var_i, dim);
                    var_i++;
                }
                if (transfer_temp) {
...
KOKKOS_INLINE_FUNCTION T estimate_dt_(const FlowStates<T>& flow_state,
    ...
    for (size_t face_idx = 0; face_idx < cell_face_ids.size(); face_idx++) {
        ...
        T dot = vx * interfaces.norm().x(i_face) + vy * interfaces.norm().y(i_face) +
                vz * interfaces.norm().z(i_face);
        T sig_vel = Ibis::abs(dot) + gas_model.speed_of_sound(flow_state.gas, cell_i);
...
void FiniteVolume<T, MemModel>::estimate_dt(Ibis::Array1D<Ibis::real>& dt,
    ...
    return Ibis::parallel_for(
        "FV::signal_frequency", num_cells, KOKKOS_LAMBDA(const size_t cell_i) {
            T local_dt = estimate_dt_(flow_state, face_fs, grid, gas_model, trans_prop,
                                      viscous, viscous_signal_factor, cell_i);
