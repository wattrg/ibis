...
template <typename T>
void apply_time_derivative(const ConservedQuantities<T>& U0,
                           ConservedQuantities<T>& U1,
                           ConservedQuantities<T>& dUdt,
                           double dt) {
    size_t n_values = U0.size();
    size_t dim = U0.dim();
    Kokkos::parallel_for("apply_time_derivative", n_values, KOKKOS_LAMBDA(const int i){
...
template void apply_time_derivative(const ConservedQuantities<double>&, ConservedQuantities<double>&, ConservedQuantities<double>&, double);
