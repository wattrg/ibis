...
public:
    ...
    CellFaces(const Id<array_layout, memory_space>& interface_ids){
        ...
        face_ids_ = view_type("CellFaces::face_ids", interface_ids.ids().size());      
        outsigns_ = view_type("CellFaces::outsigns", interface_ids.ids().size());
    ...
    int num_face_ids() const {
        return face_ids_.extent(0);
    }
...
public:
    ...
    Cells(Id<array_layout, memory_space> vertices, 
        ...
        shape_ = Field<ElemType, array_layout, memory_space>("Cell::shape", 
                                                             num_cells_);
        typename Field<ElemType, array_layout, memory_space>::mirror_type shape_mirror ("Cell::shape", num_cells_);
        for (int i = 0; i < num_cells_; i++) {
            shape_mirror(i) = shapes[i]; 
        }
        shape_.deep_copy(shape_mirror);

        volume_ = Field<T, array_layout, memory_space>("Cell::Volume", num_cells_);
        centroid_ = Vector3s<T, array_layout, memory_space>("Cell::centroids", num_cells_);
    }
    ...
    Cells(int num_cells, int num_vertex_ids, int num_face_ids) {
        ...
        faces_ = CellFaces<T, array_layout, memory_space>(num_cells, num_face_ids);
        shape_ = Field<ElemType, array_layout, memory_space>("Cell::shape", num_cells);
        volume_ = Field<T, array_layout, memory_space>("Cell::Volume", num_cells);
    ...
    KOKKOS_INLINE_FUNCTION
    const Id<array_layout, memory_space> &
    vertex_ids() const {return vertex_ids_;}
    ...
    KOKKOS_INLINE_FUNCTION
    const Field<T, array_layout, memory_space>& 
    volumes() const {return volume_;}

    KOKKOS_INLINE_FUNCTION
    Field<T, array_layout, memory_space>& volumes() {return volume_;}

    KOKKOS_INLINE_FUNCTION
    const Vector3s<T, array_layout, memory_space>& 
    centroids() const {return centroid_;}
